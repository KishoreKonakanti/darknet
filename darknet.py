...
class IMAGE(Structure):
    ...
    def shape(self):
        return (IMAGE._fields_[0], IMAGE._fields_[1])
...
def cv2detect(net, meta, image, thresh=.5, hier_thresh=.5, nms=.45):
    """
    Performs the meat of the detection
    """
    #pylint: disable= C0321
...
def cv2detect_image(net, meta, im, thresh=.5, hier_thresh=.5, nms=.45):
    num = c_int(0)
    ...
    print('PREDICT_IMAGE took:',(time.time()-st))
    letter_box = 0
    if debug: print("did prediction")
    dets = get_network_boxes(net, w, h, thresh, hier_thresh, None, 0, pnum, letter_box)
...
def performDetect(imagePath='/home/kishore/masked.jpg', thresh= 0.25,
    ...
    try:
        ...
        print("*** "+str(len(detections))+" Results, color coded by confidence ***") if args.debug else None 
        imcaption = []
...
def performBatchDetect(thresh= 0.25,
    ...
    img_samples = [f'batch/1.jpg' for i in range(batch_size)]
